...
const FileUpload = () => { 
  ...
  const handleFile = async (e) => {
    ...
    if (files.length > 5) {
      errorMessages.push("You are only allowed to upload at most 5 files.");
    }

    files.forEach((file) => {
      if (file.size < minSize || file.size > maxSize) {
        errorMessages.push("File size should be between 50 KB to 1 MB.");
      } else if (!file.type.startsWith("image/")) {
        errorMessages.push("Only image files are accepted.");
      } else {
        validFiles.push({
    ...
    if (errorMessages.length > 0) {
      setError(true);
      setEMessage([...new Set(errorMessages)]); // Remove duplicate error messages
    } else {
      setError(false);
      setfile(validFiles);
    }
    ...
    <div className="h-screen w-screen flex justify-center">
      <div className="h-4/5 border-[.1rem] border-black w-4/5 m-auto rounded-lg grid grid-flow-row grid-rows-[20%_auto]">
        ...
        >
          ...
          {Error &&
            [...new Set(EMessage)].map((msg, key) => (
              <div key={key}>{toast.error(msg)}</div>
            ))}
          {file && file.length > 0 ? (
            <div className="h-full flex flex-col border-blue-50 p-4 gap-4">
              ...
              <div className='flex justify-evenly'>
                <button type="submit" className='pl-4 pr-4 bg-black text-white pt-2 pb-2 rounded-xl' onClick={(e) =>{HandleSubmit(e)}}>Submit</button>
                <button type="cancel" className='pl-4 pr-4 bg-black text-white pt-2 pb-2 rounded-xl' onClick={(e) => {HandleDiscard(e)}}>Discard</button>
              </div>
            </div>
          ) : (
            <p className="text-gray-500 flex justify-center">No files uploaded yet.</p>
          )}
        </div>
      </div>
    </div>
